...
async function runFullScanAnalysis(images){
  ...
  try{
    ...
    const returned={...j.analysis,_analysisEngine:j.fallback?'local-scan-fallback':'gemini',_analysisModel:j.model||'',_keySlot:j.keySlot||0};
...
async function refreshHairAnalysis(){
  ...
  setHairAnalysisError('');
  setProfileReady(false);
  setScanAnalysisStage('Validating four views...');
  sessionStorage.removeItem('clipProfileReady');
  await runFullScanAnalysis(scanShots);
}

useEffect(()=>{
  const isFallback=hairAnalysis?._analysisEngine==='local-scan-fallback';
  const hasFour=(scanShots||[]).filter(Boolean).length===4;
  const retryMarker='clipFallbackRetry-key2-v2';
  if(step===2&&isFallback&&hasFour&&!hairAnalyzing&&sessionStorage.getItem(retryMarker)!=='1'){
